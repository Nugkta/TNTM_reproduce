...
class TNTM_bow():
  def __init__(
    ...
    :param str device: "cpu" or "cuda". If "none", use gpu if available, else cpu
  ...
  def fit(self, corpus: list, vocab: list, embeddings: torch.Tensor):
    """
    ...
    :param torch.Tensor embeddings: Embeddings of each vocabulary where the i-th row is the embedding of the i-th word in the vocabulary. Has shape (len(vocab), embedding_dim)
    """
    ...
    for i, doc in tqdm(enumerate(corpus_idx)):
      for word in doc:
        bow_ten[i, word] +=1
    ...
    train_ds, val_ds, test_ds = TNTM_inference.train_test_split(self.bow_ten, 1 - self.validation_set_size, self.validation_set_size, self.batch_size)
